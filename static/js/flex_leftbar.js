...
const workspace_list = document.getElementById('workspace_list');

let isDragging = false;


if (folder_list && folders_toggle && folder_Arrow) {
    ...
    folders_toggle.addEventListener('click', function() {
        if (isOpen) {
            folder_list.style.opacity = '0';
            setTimeout(function() { folder_list.style.display = 'none'; }, 300);
            folder_Arrow.className = 'bi bi-caret-down-fill';
            localStorage.setItem('folderOpen', 'false');
        } else {
            folder_list.style.maxHeight = '300px';
            folder_list.style.display = '';
            setTimeout(function() { folder_list.style.opacity = '1'; }, 10);
            folder_Arrow.className = 'bi bi-caret-up-fill';
    ...
    if (workspace_list) {
        workspace_list.addEventListener('scroll', function() {
            localStorage.setItem('workspaceScrollPos', workspace_list.scrollTop);
        });

        window.addEventListener('load', function() {
            const savedScrollPos = localStorage.getItem('workspaceScrollPos');
            if (savedScrollPos) {
                workspace_list.scrollTop = parseInt(savedScrollPos)
            }
        });
    }
}
